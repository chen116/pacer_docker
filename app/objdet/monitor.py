...
for x in (client.containers.list(all=True)):
	print("rm:",x.name)
	x.remove(force=True)
vicid={"VIC_SHM_ID":str(1000),"DISPLAY":os.environ['DISPLAY']}
# vicid2={"VIC_SHM_ID":str(3000)}

mnt = [docker.types.Mount('/foo', '/root/pacer_docker/ipc/',type='bind'),docker.types.Mount('/tmp/.X11-unix:ro', '/tmp/.X11-unix/',type='bind')]
# mnt2 = docker.types.Mount('/foo', '/root/pacer_docker/ipc/',type='bind')
...
# from command line to see stdout... $ sudo docker logs v1(or whatever the name is)
